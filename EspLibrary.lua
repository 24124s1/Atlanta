...
local function isSquadmate(player)
    if not player or player == Services.LocalPlayer then return false end
    local character = player.Character
    return character ~= nil and character:FindFirstChild("SquadBillboard") ~= nil
end

local function isTeammate(player)
    if not player or player == Services.LocalPlayer then return false end
    if player.Team ~= nil and player.Team == Services.LocalPlayer.Team then
        return true
    end
    if isSquadmate(player) then
        return true
    end
    return false
end
...
local function CreatePlayerESP(player)
    if espModule._drawings[player] then return end
...
local function RemoveESP(player)
    local data = espModule._drawings[player]
    ...
    espModule._drawings[player] = nil
    if espModule._highlights[player] then
        espModule._highlights[player]:Destroy()
...
local function espLoop()
    ...
    for _, player in ipairs(Services.Players:GetPlayers()) do
        ...
        local data = espModule._drawings[player]
        local teammateStatus = isTeammate(player)
        local cfg = teammateStatus and espModule.Settings.Teammate or espModule.Settings.Enemy
        
        if not cfg.Enabled then
            HideESP(data)
            if espModule._highlights[player] then espModule._highlights[player].Enabled = false end
            continue
        end
        ...
        if cfg.Chams then
            ...
            if not hl then
                ...
                espModule._highlights[player] = hl
            end
            ...
            if espModule._highlights[player] then
                espModule._highlights[player].Enabled = false
            end
        end
    end
end
...
function espModule:Stop()
    ...
    for player, data in pairs(self._drawings) do
        RemoveESP(player)
    end
    self._drawings = {}
    self._highlights = {}
