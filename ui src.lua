...
-- 				Name = "dropdown",
-- 				ZIndex = 2,
-- 				Size = cfg.inline and dim2(0, 90, 0, 12) or dim2(1, -8, 0, 12),
-- 				BorderSizePixel = 0,
-- 				BackgroundTransparency = 1,
-- 				TextXAlignment = Enum.TextXAlignment.Left,
-- 				AutomaticSize = Enum.AutomaticSize.Y,
	...
	local config_holder 
		...
		function library:window(properties)
			...
			function window.set_menu_visibility(bool) 
				...
				local watermark = library:watermark({default = string.format("Catware.top | Premium | %s", gameName)})  

				task.spawn(function()
    				while task.wait(1) do 
        				watermark.change_text(string.format("Catware.top | Premium | %s", gameName))
    				end 
				end)
